...
format (directory, ZIP, TAR, fixture JSON) and writes ``anr_ai_context/``
containing ``index.json`` and per-ANR ``anr_analysis.md`` files (AI
...
After running, the agent should read ``anr_ai_context/<anr-id>/anr_analysis.md`` and
analyze the evidence to produce a root-cause report.
"""
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", default="anr_ai_context", help="Output directory (default: anr_ai_context)")
    ...
    print(f"[✓] {summary['groupCount']} ANR analysis file(s) written under {args.out_dir}/")
    ...
    if pending_groups:
        ...
        print(f"Next: fill each {args.out_dir}/<anr-id>/anr_analysis.md in order: Trace → EventLog → Logcat/AnrManager → Final ANR.")
        ...
        print(f"Next: review each {args.out_dir}/<anr-id>/anr_analysis.md final ANR section and deliver the report.")
